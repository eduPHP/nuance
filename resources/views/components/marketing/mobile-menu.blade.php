...
<details class="relative md:hidden">
    ...
    <div class="absolute right-0 mt-2 w-56 rounded-xl border border-border bg-popover p-2 shadow-lg">
        @foreach ($links as $link)
            ...
            <a href="{{ $link['href'] }}" @if ($shouldNavigate) wire:navigate @endif class="block rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-secondary hover:text-foreground">{{ $link['label'] }}</a>
        @endforeach
